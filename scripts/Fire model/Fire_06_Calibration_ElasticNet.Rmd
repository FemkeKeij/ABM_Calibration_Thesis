...
```{r}
p1 <- PlotPredTrueDensity(elastic_results, n = 1584)
p2 <- PlotPredTrueDirection(elastic_results, n = 1584)
p3 <- PlotPredTrueBurn(elastic_results, n = 1584)
...
```{r}
PlotPredTrueDirection(elastic_results, n = 1584)

ggsave('figures/fire_elasticnet_predtrue_direction.pdf')
```
...
```{r}
PlotErrorsDensity(elastic_errors, n_plot = c(99, 792, 1584))

ggsave('figures/fire_elasticnet_density_metrics.pdf')
```
F1, kappa score, and MCC for directions:
```{r}
PlotErrorsDirections(elastic_errors, n_plot = c(99, 792, 1584))

ggsave('figures/fire_elasticnet_directions_metrics.pdf')
```
### Outcome variable
