...
TimeSeriesSummarising <- function(data, full = FALSE,
  ...
  if(full & timesteps){
    data_return <- data %>%
      ...
      filter(ticks_noise %% 10 == 0 | ticks %% 10 == 0)
...
fire_summary_timeseries <- fire_summary_noise %>%
...
# unsummarised runs, full time series
fire_full_timeseries <- fire_full_clean %>%
  mutate(sample_size = 198,
         sample_method = 'fill',
         summarise_runs = 'full',
         datapoints = 'timeseries')
...
fire_training <- bind_rows(fire_summary_timeseries,
                           fire_summary_timesteps,
                           fire_summary_endpoints,
                           fire_full_timeseries,
                           fire_full_timesteps,
                           fire_full_endpoints) %>%
  select(- mean_burn_percentage)
