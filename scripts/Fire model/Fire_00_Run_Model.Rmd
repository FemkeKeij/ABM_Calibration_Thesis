...
```{python}
parameter_space_df.to_csv("C:/Users/Femke Keij/OneDrive/Thesis/R projects/LU thesis local/data/raw/fire_output_complete.csv")
```
...
```{r}
fire_output <- read_csv("data/raw/fire_output_complete.csv",
                        col_names = TRUE,
                        col_select = 2:4)
```
...
fire_data %>%
  write_csv(file = 'data/processed/fire_data_summary.csv')
```
...
fire_output %>%
  slice(-ind) %>%
  write_csv(file = 'data/processed/fire_training.csv')
```