...
# for the reduced rank regression
library(rrpack)
```
...
mrrr(Y = Y, X = X, is.pca = NULL, family = list(gaussian(link = 'identity'),
                                                binomial(link = 'logit')),
     list(1, 2))
```
...
fit <- build_forest_predict(trainX = X, trainY = Y, n_tree = 100,
                     m_feature = 1, min_leaf = 5, testX = testX)

write_csv(as.data.frame(fit), 'data/processed/fire_mrf_fitpred.csv')
```
