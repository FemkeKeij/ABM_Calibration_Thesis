...
Some of the data columns are so similar that the regression cannot distinguish between them (LOOK UP WHAT THIS IS CALLED AGAIN). For that reason, I cap the timeseries at timestep 650. This removes part of the problem.
...
10 timesteps, not summarised
```{r}
# 10 timesteps, not summarised, data without noise
data_10_full_clean <- fire_training %>%
  filter(datapoints == 'timesteps',
         summarise_runs == 'full',
         ticks %% 10 == 0,
         ticks < 650) %>%
  select(ticks, density, directions, burn_percentage,
...
fit_data_10_full_clean <-
  FireSimpleRegressionFitting(data_10_full_clean,
                              data_noise = NA)
errors_data_10_full_clean <- 
  FireComputeErrors(fit_data_10_full_clean,
                    sample_size = 198,
                    sample_method = 'full',
                    summarise_runs = 'full',
                    noise = 'clean',
...
errors <- errors_data_10_full_clean

# 10 timesteps, not summarised, data with output noise
data_10_full_outputnoise <- fire_training %>%
  filter(datapoints == 'timesteps',
         summarise_runs == 'full',
         ticks %% 10 == 0,
         ticks < 650) %>%
  select(ticks, density, directions, burn_percentage_noise,
         mean_burn, min_burn, sd_burn, trend_burn) %>%
  pivot_wider(names_from = ticks,
...
fit_data_10_full_outputnoise <-
  FireSimpleRegressionFitting(data_10_full_clean,
                              data_10_full_outputnoise)
errors_data_10_full_outputnoise <- 
  FireComputeErrors(fit_data_10_full_outputnoise,
                    sample_size = 198,
                    sample_method = 'full',
                    summarise_runs = 'full',
                    noise = 'output noise',
...
errors <- errors %>%
  add_row(errors_data_10_full_outputnoise)

# 10 timesteps, not summarised, data with output and temporal noise
data_10_full_doublenoise <- fire_training %>%
  filter(datapoints == 'timesteps',
         summarise_runs == 'full',
         ticks_noise %% 10 == 0,
         ticks_noise < 650 & ticks_noise > 0) %>%
  select(ticks_noise, density, directions, burn_percentage_noise,
         mean_burn, min_burn, sd_burn, trend_burn) %>%
...
fit_data_10_full_doublenoise <-
  FireSimpleRegressionFitting(data_10_full_clean,
                              data_10_full_doublenoise)
errors_data_10_full_doublenoise <- 
  FireComputeErrors(fit_data_10_full_doublenoise,
                    sample_size = 198,
                    sample_method = 'full',
                    summarise_runs = 'full',
                    noise = 'double noise',
...
errors <- errors %>%
  add_row(errors_data_10_full_doublenoise)
```

10 timesteps, summarised
```{r}
# 10 timesteps, summarised, data without noise
data_10_sum_clean <- fire_training %>%
  filter(datapoints == 'timesteps',
...
fit_data_10_sum_clean <-
  FireSimpleRegressionFitting(data_10_sum_clean,
                              data_noise = NA)
errors_data_10_sum_clean <- 
  FireComputeErrors(fit_data_10_sum_clean,
                    sample_size = 198,
                    sample_method = 'full',
                    summarise_runs = 'summarised',
                    noise = 'clean',
...
errors <- errors %>%
  add_row(errors_data_10_sum_clean)

# 10 timesteps, summarised, data with output noise
data_10_sum_outputnoise <- fire_training %>%
  filter(datapoints == 'timesteps',
         summarise_runs == 'summarised',
         ticks %% 10 == 0,
         ticks < 650) %>%
...
fit_data_10_sum_outputnoise <-
  FireSimpleRegressionFitting(data_10_sum_clean,
                              data_10_sum_outputnoise)
errors_data_10_sum_outputnoise <- 
  FireComputeErrors(fit_data_10_sum_outputnoise,
                    sample_size = 198,
                    sample_method = 'full',
                    summarise_runs = 'summarised',
                    noise = 'output noise',
...
errors <- errors %>%
  add_row(errors_data_10_sum_outputnoise)

# 10 timesteps, summarised, data with output and temporal noise
data_10_sum_doublenoise <- fire_training %>%
  filter(datapoints == 'timesteps',
         summarise_runs == 'summarised',
         ticks_noise %% 10 == 0,
         ticks_noise < 650 & ticks_noise > 0) %>%
...
fit_data_10_sum_doublenoise <-
  FireSimpleRegressionFitting(data_10_sum_clean,
                              data_10_sum_doublenoise)
errors_data_10_sum_doublenoise <- 
  FireComputeErrors(fit_data_10_sum_doublenoise,
                    sample_size = 198,
                    sample_method = 'full',
                    summarise_runs = 'summarised',
                    noise = 'double noise',
...
errors <- errors %>%
  add_row(errors_data_10_sum_doublenoise)
```

20 timesteps, not summarised
```{r}
# 20 timesteps, not summarised, data without noise
data_20_full_clean <- fire_training %>%
  filter(datapoints == 'timesteps',
         summarise_runs == 'full',
         ticks %% 20 == 0,
         ticks < 650) %>%
...
fit_data_20_full_clean <-
  FireSimpleRegressionFitting(data_20_full_clean,
                              data_noise = NA)
errors_data_20_full_clean <- 
  FireComputeErrors(fit_data_20_full_clean,
                    sample_size = 198,
                    sample_method = 'full',
                    summarise_runs = 'full',
                    noise = 'clean',
...
# dataframe to store all errors
errors <- errors %>%
  add_row(errors_data_20_full_clean)

# 20 timesteps, not summarised, data with output noise
data_20_full_outputnoise <- fire_training %>%
  filter(datapoints == 'timesteps',
         summarise_runs == 'full',
         ticks %% 20 == 0,
         ticks < 650) %>%
...
fit_data_20_full_outputnoise <-
  FireSimpleRegressionFitting(data_20_full_clean,
                              data_20_full_outputnoise)
errors_data_20_full_outputnoise <- 
  FireComputeErrors(fit_data_20_full_outputnoise,
                    sample_size = 198,
                    sample_method = 'full',
                    summarise_runs = 'full',
                    noise = 'output noise',
...
errors <- errors %>%
  add_row(errors_data_20_full_outputnoise)

# 20 timesteps, not summarised, data with output and temporal noise
data_20_full_doublenoise <- fire_training %>%
  filter(datapoints == 'timesteps',
         summarise_runs == 'full',
         ticks_noise %% 20 == 0,
         ticks_noise < 650 & ticks_noise > 0) %>%
...
fit_data_20_full_doublenoise <-
  FireSimpleRegressionFitting(data_20_full_clean,
                              data_20_full_doublenoise)
errors_data_20_full_doublenoise <- 
  FireComputeErrors(fit_data_20_full_doublenoise,
                    sample_size = 198,
                    sample_method = 'full',
                    summarise_runs = 'full',
                    noise = 'double noise',
...
errors <- errors %>%
  add_row(errors_data_20_full_doublenoise)
```

20 timesteps, summarised
```{r}
# 20 timesteps, summarised, data without noise
data_20_sum_clean <- fire_training %>%
  filter(datapoints == 'timesteps',
         summarise_runs == 'summarised',
         ticks %% 20 == 0,
         ticks < 650) %>%
...
fit_data_20_sum_clean <-
  FireSimpleRegressionFitting(data_20_sum_clean,
                              data_noise = NA)
errors_data_20_sum_clean <- 
  FireComputeErrors(fit_data_20_sum_clean,
                    sample_size = 198,
                    sample_method = 'full',
                    summarise_runs = 'summarised',
                    noise = 'clean',
...
errors <- errors %>%
  add_row(errors_data_20_sum_clean)

# 20 timesteps, summarised, data with output noise
data_20_sum_outputnoise <- fire_training %>%
  filter(datapoints == 'timesteps',
         summarise_runs == 'summarised',
         ticks %% 20 == 0,
         ticks < 650) %>%
...
fit_data_20_sum_outputnoise <-
  FireSimpleRegressionFitting(data_20_sum_clean,
                              data_20_sum_outputnoise)
errors_data_20_sum_outputnoise <- 
  FireComputeErrors(fit_data_20_sum_outputnoise,
                    sample_size = 198,
                    sample_method = 'full',
                    summarise_runs = 'summarised',
                    noise = 'output noise',
...
errors <- errors %>%
  add_row(errors_data_20_sum_outputnoise)

# 20 timesteps, summarised, data with output and temporal noise
data_20_sum_doublenoise <- fire_training %>%
  filter(datapoints == 'timesteps',
         summarise_runs == 'summarised',
         ticks_noise %% 20 == 0,
         ticks_noise < 650 & ticks_noise > 0) %>%
...
fit_data_20_sum_doublenoise <-
  FireSimpleRegressionFitting(data_20_sum_clean,
                              data_20_sum_doublenoise)
errors_data_20_sum_doublenoise <- 
  FireComputeErrors(fit_data_20_sum_doublenoise,
                    sample_size = 198,
                    sample_method = 'full',
                    summarise_runs = 'summarised',
                    noise = 'double noise',
...
errors <- errors %>%
  add_row(errors_data_20_sum_doublenoise)
```
...
# there's no tick noise here, since we only use endpoints
```
