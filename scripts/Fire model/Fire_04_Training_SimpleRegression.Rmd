...
The summarised data does not contain enough rows to allow for computing the linear regression. Therefore, I only use the non-summarised version (which as shown in the first script is not an issue in this case).
In addition, in the latest timesteps the data are too similar for the linear regression to work on them (LOOK UP WHAT THIS IS CALLED AGAIN). Therefore, I cap the timeseries at timestep 650.
```{r}
data <- fire_training %>%
  filter(datapoints == 'timesteps',
         summarise_runs == 'full',
         # if I want to work with the noise version, use ticks_noise here
         # actually I'd need both, since only the test data should have noise, I think I'd have to make 2 separate dataframes in that case
         ticks %% 20 == 0,
         ticks < 650) %>%
  # if I want to work with the noise version, use burn_percentage_noise and/or ticks_noise here instead of burn_percentage and/or ticks
  select(ticks, density, directions, burn_percentage,
         mean_burn, min_burn, sd_burn, trend_burn) %>%
  # pivot wider to make it easier to use the timesteps as
  # predictors in the linear regression
  pivot_wider(names_from = ticks,
              values_from = burn_percentage)

# some simulations end earlier than others, extend the data
# so that each simulation has all timesteps, where added timesteps
# take on the burn_percentage value form earlier timesteps
...
# going to have to adapt this also for the ticks_noise version

out <- FireSimpleRegressionFitting(data)
```

```{r}
FireComputeErrors(out, sample_size = 198,
                  sample_method = 'full',
                  summarise_runs = 'full',
                  noise = 'clean',
