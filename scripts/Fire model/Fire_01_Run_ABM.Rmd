...
```{python}
dictionary = {'densities': [5, 50, 95], 'directions': [4, 8]}
...
for i in range(0,len(param_comb)):
  ...
  directions = str(param_comb['density'][i])
  ...
  for x in range(0, 50):
    ...
    # store
    out_df = out_df.append({'density': density, 'directions': directions, 'run': x + 1, 'burn_percentage': burn_perc, 'tick': ticks}, ignore_index = True)
...
p2 <- mean_conv_burn %>%
  ...
  geom_vline(xintercept = mean(mean_conv_burn$run),
             colour = 'darkgreen', lty = 2) +
  geom_vline(xintercept = median(mean_conv_burn$run),
             colour = 'darkblue', lty = 2) +
  annotate(geom = 'text', label = 'mean',
           x = mean(mean_conv_burn$run) + 1, y = 20,
           colour = 'darkgreen', angle = 90) +
  annotate(geom = 'text', label = 'median',
           x = median(mean_conv_burn$run) - 1, y = 20,
...
p2 <- cv_conv_burn %>%
  ...
  geom_vline(xintercept = median(cv_conv_burn$run),
             colour = 'darkblue', lty = 2) +
  annotate(geom = 'text', label = 'mean',
           x = mean(cv_conv_burn$run) + 1, y = 20,
           colour = 'darkgreen', angle = 90) +
  annotate(geom = 'text', label = 'median',
           x = median(cv_conv_burn$run) - 1, y = 20,
           colour = 'darkblue', angle = 90) +
  theme_minimal() +
  labs(y = 'count',
...
numruns <- numruns %>% 
  ...
  mutate(windowed_var = slider::slide_dbl(burn_percentage,
                                          var, .before = 5,
                                          .after = 5),
         relative_outer_var = windowed_var / max(windowed_var))
