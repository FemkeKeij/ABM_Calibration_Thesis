...
```{r}
p1 <- PlotPredTrueDensity(lhs_results, n = 1584)
p2 <- PlotPredTrueDirection(lhs_results, n = 1584)
p3 <- PlotPredTrueBurn(lhs_results, n = 1584)
...
```{r}
PlotPredTrueDirection(lhs_results, n = 1584)

ggsave('figures/fire_LHS_predtrue_direction.pdf')
```
Predicted vs. true burn percentage (largest sample size only):
```{r}
PlotPredTrueBurn(lhs_results, n = 1584)

ggsave('figures/fire_LHS_predtrue_burn.pdf')
```
### Parameters
...
```{r}
PlotErrorsDensity(lhs_errors, n_plot = c(99, 792, 1584))

ggsave('figures/fire_LHS_RMSE_density.pdf')
```
F1, kappa score, and MCC for directions:
```{r}
PlotErrorsDirections(lhs_errors, n_plot = c(99, 792, 1584))

ggsave('figures/fire_LHS_directions_metrics.pdf')
```
### Outcome variable
